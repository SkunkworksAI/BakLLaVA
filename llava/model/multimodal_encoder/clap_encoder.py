...
from transformers import ClapModel, ClapProcessor, ClapAudioConfig, ClapAudioModel, AutoFeatureExtractor, ClapConfig, ClapAudioConfig, ClapAudioModelWithProjection
...
class CLAPAudioTower(nn.Module):
    ...
    def load_model(self):
        config = ClapAudioConfig.from_pretrained(self.audio_tower_name)

        self.audio_tower = ClapAudioModel(config)
        # self.audio_tower = ClapAudioModelWithProjection.from_pretrained(self.audio_tower_name)
    ...
    def feature_select(self, audio_forward_outs):
        ...
        bs, embed_dim, a, b = audio_features.shape
        return audio_forward_outs.reshape(-1, embed_dim)
